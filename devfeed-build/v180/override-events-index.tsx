...
import { freshFirst, markFreshRead, useFreshFeed, useFreshInbox } from '@/freshness';
...
function deadlineEnd(deadline: string | null | undefined) { return localBoundary(deadline, true); }
function isDeadlineClosed(event: EventItem) { const end = deadlineEnd(event.deadline); return Boolean(end && end.getTime() < Date.now()); }
function isEventEnded(event: EventItem) {
  ...
  const assumedEnd = /^\d{4}-\d{2}-\d{2}$/.test(startRaw) ? start : new Date(start.getTime() + 6 * 60 * 60 * 1000);
  return assumedEnd.getTime() < Date.now();
}
function isActionable(event: EventItem) { return !isEventEnded(event) && !isDeadlineClosed(event); }
...
function EventRow({ event, compact, isNew = false }: { event: EventItem; compact: boolean; isNew?: boolean }) {
  const deadline = dday(event.deadline);
  const status = statusLabel(event.userStatus);
  const approximate = eventPrecision(event) !== 'EXACT';
  const open = () => {
    void markFreshRead('event', event.id);
    router.push(`/event/${event.id}`);
  };
  return (
    <Pressable onPress={open} style={({ pressed }) => [styles.eventRow, compact && styles.eventRowCompact, isNew && styles.eventRowNew, pressed && { opacity: 0.62 }]}>
      <View style={[styles.dateTile, compact && styles.dateTileCompact, isNew && styles.dateTileNew]}>
      ...
      <View style={styles.eventBody}>
        <View style={styles.eventTopLine}>
          <View style={styles.tagLine}>{isNew ? <Badge tone="purple">NEW</Badge> : null}{event.categories.slice(0, compact ? 1 : 2).map(c => <Badge key={c} tone="purple">{c}</Badge>)}</View>
...
export default function EventsScreen() {
  ...
  const unread = useFreshFeed('event', active.map(e => ({ id: e.id, title: e.title, route: `/event/${e.id}` })));
  ...
  const sortedFiltered = useMemo(() => freshFirst(filtered, unread), [filtered, unread]);
  const freshItems = sortedFiltered.filter(e => unread.has(String(e.id)));
  ...
  const withoutFresh = sortedFiltered.filter(e => !unread.has(String(e.id)));
  const featured = withoutFresh.filter(e => e.importance === 'HIGH').slice(0, 4);
  const rest = withoutFresh.filter(e => !featured.some(x => x.id === e.id));
  ...
  return (
    <Page style={{ paddingTop: 10 }}>
      <View style={[styles.workspaceHero, compact && styles.workspaceHeroCompact]}>
        ...
        <View style={styles.heroTop}>
          ...
          <View style={styles.heroActions}>
            <Pressable onPress={() => router.push('/fresh-inbox')} style={styles.settingsButton}><Text style={styles.settingsButtonText}>소식{inbox.unreadCount > 0 ? ` ${inbox.unreadCount}` : ''}</Text></Pressable>
      ...
      {freshItems.length > 0 ? <>
        <SectionTitle title="새로 찾았어요" subtitle="마지막 확인 이후 새로 수집된 행사부터 먼저 보여줘요." right={<Badge tone="purple">{freshItems.length} NEW</Badge>} />
        <View style={styles.freshWrap}>{freshItems.map(e => <EventRow compact={compact} key={`fresh-${e.id}`} event={e} isNew />)}</View>
      </> : null}
      ...
      {urgent.length > 0 ? (
        <Card style={styles.agendaCard}>
          ...
          {urgent.map((e, i) => (
            <Pressable key={e.id} onPress={() => { void markFreshRead('event', e.id); router.push(`/event/${e.id}`); }} style={styles.agendaRow}>
...
const styles = StyleSheet.create({
  ...
  heroActions: { gap: 7, alignItems: 'flex-end', flexShrink: 0 },
  ...
  settingsButtonText: { color: colors.white, fontSize: 11, fontWeight: '900' },
  heroStats: { marginTop: 'auto', paddingTop: 24, flexDirection: 'row', alignItems: 'center' },
  heroStat: { flex: 1, gap: 3, minWidth: 0 }, heroStatNumber: { color: colors.white, fontSize: 23, fontWeight: '900' }, heroStatLabel: { color: '#DDD3F5', fontSize: 11, fontWeight: '700' }, heroStatDivider: { width: 1, height: 34, backgroundColor: '#FFFFFF2D', marginHorizontal: 8 },
  freshWrap: { backgroundColor: '#FAF8FF', borderRadius: radius.lg, borderWidth: 1, borderColor: '#DDD1FF', overflow: 'hidden' },
  agendaCard: { paddingVertical: 14 }, agendaRow: { minHeight: 56, flexDirection: 'row', alignItems: 'center', gap: 11, borderTopWidth: 1, borderTopColor: colors.line, paddingTop: 11, marginTop: 2 }, agendaIndex: { width: 28, height: 28, borderRadius: 9, backgroundColor: colors.accentSoft, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }, agendaIndexText: { color: colors.accentDark, fontSize: 12, fontWeight: '900' }, agendaTitle: { color: colors.text, fontSize: 14, fontWeight: '800' }, agendaMeta: { color: colors.muted, fontSize: 11, fontWeight: '600', marginTop: 3 }, smallArrow: { color: colors.muted, fontSize: 17, fontWeight: '700', flexShrink: 0 },
  filterRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, filterChip: { paddingHorizontal: 13, paddingVertical: 9, borderRadius: radius.pill, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line }, filterChipActive: { backgroundColor: colors.text, borderColor: colors.text }, filterText: { color: colors.muted, fontSize: 12, fontWeight: '800' }, filterTextActive: { color: colors.white }, countText: { color: colors.muted, fontSize: 12, fontWeight: '800' },
  featuredWrap: { backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.line, overflow: 'hidden' }, listWrap: { backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.line, overflow: 'hidden' },
  eventRow: { flexDirection: 'row', gap: 13, padding: 15, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, eventRowCompact: { paddingHorizontal: 12, gap: 10 }, eventRowNew: { backgroundColor: '#FAF8FF' },
  dateTile: { width: 50, height: 58, borderRadius: 14, backgroundColor: colors.accentSoft2, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }, dateTileCompact: { width: 44, height: 54 }, dateTileNew: { backgroundColor: '#E9E0FF' }, dateMonth: { color: colors.accentDark, fontSize: 9, fontWeight: '900' }, dateDay: { color: colors.accentDark, fontSize: 24, lineHeight: 27, fontWeight: '900' },
  eventBody: { flex: 1, minWidth: 0, gap: 6 }, eventTopLine: { flexDirection: 'row', gap: 8, justifyContent: 'space-between', alignItems: 'center' }, tagLine: { flexDirection: 'row', flexWrap: 'wrap', gap: 5, flex: 1, minWidth: 0 }, deadline: { color: colors.muted, fontSize: 11, fontWeight: '900', flexShrink: 0 }, eventTitle: { color: colors.text, fontSize: 17, lineHeight: 23, fontWeight: '900', letterSpacing: -0.35 }, eventMeta: { color: colors.muted, fontSize: 11, lineHeight: 17, fontWeight: '600' }, eventBottomLine: { flexDirection: 'row', alignItems: 'flex-end', gap: 8 }, eventSummary: { color: '#5F5F6B', fontSize: 12, lineHeight: 18, flex: 1, minWidth: 0 },
});
