...
export async function downloadAndInstallUpdate(info: UpdateInfo) {
  ...
  const result = await FileSystem.downloadAsync(info.downloadUrl, destination);
    ...
    Alert.alert(
      ...
      [
        { text: '나중에', style: 'cancel' },
        {
          text: '업데이트',
          onPress: () => {
            void downloadAndInstallUpdate(info).catch((error) => {
