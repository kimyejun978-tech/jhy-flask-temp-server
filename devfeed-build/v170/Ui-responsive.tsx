...
import { ActivityIndicator, Pressable, ScrollView, StyleSheet, Text, View, useWindowDimensions, type StyleProp, type TextStyle, type ViewStyle } from 'react-native';
...
export function Page({ children, style }: { children: ReactNode; style?: StyleProp<ViewStyle> }) {
  ...
  const horizontal = width < 350 ? 12 : width < 430 ? 16 : 20;
  return (
    <ScrollView
      style={styles.page}
      contentContainerStyle={[
        ...
        style,
        {
          ...
          paddingBottom: Math.max(insets.bottom + 110, 118),
        },
      ]}
    ...
    >
      <View style={[styles.contentWidth, width >= 700 && styles.contentWidthTablet]}>{children}</View>
    </ScrollView>
  );
}

export function Card({ children, onPress, style }: { children: ReactNode; onPress?: () => void; style?: StyleProp<ViewStyle> }) {
  if (onPress) return <Pressable onPress={onPress} style={({ pressed }) => [styles.card, style, pressed && { opacity: 0.72 }]}>{children}</Pressable>;
  return <View style={[styles.card, style]}>{children}</View>;
}

export function Button({ label, onPress, variant = 'primary', disabled = false, loading = false, accessibilityLabel }: { label: string; onPress: () => void; variant?: 'primary' | 'secondary' | 'ghost' | 'danger'; disabled?: boolean; loading?: boolean; accessibilityLabel?: string }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={accessibilityLabel ?? label} accessibilityState={{ disabled: inactive, busy: loading }} disabled={inactive} onPress={onPress} style={({ pressed }) => [styles.button, buttonStyle, (pressed || inactive) && { opacity: 0.58 }]}>{loading ? <ActivityIndicator color={variant === 'primary' ? colors.white : colors.accentDark} /> : null}<Text style={textStyle}>{label}</Text></Pressable>;
}
...
export function Header({ title, eyebrow, subtitle, right }: { title: string; eyebrow?: string; subtitle?: string; right?: ReactNode }) {
  return <View style={styles.headerRow}><View style={{ flex: 1, minWidth: 0 }}>{eyebrow ? <Text style={styles.eyebrow}>{eyebrow}</Text> : null}<Text style={styles.pageTitle}>{title}</Text>{subtitle ? <Text style={styles.subtitle}>{subtitle}</Text> : null}</View>{right}</View>;
}

export function SectionTitle({ title, subtitle, right }: { title: string; subtitle?: string; right?: ReactNode }) {
  return <View style={styles.sectionRow}><View style={{ flex: 1, minWidth: 0 }}><Text style={styles.sectionTitle}>{title}</Text>{subtitle ? <Text style={styles.sectionSubtitle}>{subtitle}</Text> : null}</View>{right}</View>;
}
...
const styles = StyleSheet.create({
  ...
  pageContent: { gap: 0 },
  contentWidth: { width: '100%', alignSelf: 'center', gap: 16 },
  contentWidthTablet: { maxWidth: 680 },
  card: { backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.line, padding: 16, gap: 10 },
  button: { minHeight: 50, borderRadius: 15, paddingHorizontal: 17, flexDirection: 'row', gap: 9, alignItems: 'center', justifyContent: 'center' },
  ...
  secondaryButtonText: { color: colors.accentDark, fontSize: 14, fontWeight: '900', textAlign: 'center' },
  dangerButtonText: { color: colors.red, fontSize: 14, fontWeight: '900', textAlign: 'center' },
  badge: { paddingHorizontal: 9, paddingVertical: 5, borderRadius: radius.pill, alignSelf: 'flex-start', maxWidth: '100%' },
  ...
  subtitle: { color: colors.muted, fontSize: 14, lineHeight: 21, fontWeight: '500', marginTop: 6, maxWidth: 420 },
