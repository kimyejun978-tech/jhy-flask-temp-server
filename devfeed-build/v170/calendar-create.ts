...
export function isCalendarAiRequestCancelled(error: unknown) {
...
export async function parseNaturalCalendarTextWithAI(input: string, signal?: AbortSignal): Promise<AiCalendarParseResult> {
  ...
  let result: AiCalendarResponse;
  try {
    result = await api<AiCalendarResponse>('/v1/calendar/parse', {
      method: 'POST',
    ...
    if (controller.signal.aborted) throw new Error('AI 응답이 늦어지고 있어요. 다시 시도하거나 직접 입력해 주세요.');
    const message = error instanceof Error ? error.message : String(error);
    if (/\b50[234]\b/.test(message)) throw new Error('AI가 잠시 바빠요. 잠깐 뒤 다시 시도하거나 직접 입력해 주세요.');
    throw new Error('AI에 연결하지 못했어요. 네트워크를 확인하고 다시 시도해 주세요.');
  } finally {
