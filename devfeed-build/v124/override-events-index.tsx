...
export default function EventsScreen(){const [category,setCategory]=useState('전체');const all=useEvents();const schedule=useSchedule();const items=(all.data?.items??[]).filter(e=>category==='전체'||e.categories.some(c=>c.toLowerCase().includes(category.toLowerCase())||category.toLowerCase().includes(c.toLowerCase())));return <Page><Header title="행사" right={<Pressable onPress={()=>router.push('/settings')}><Text style={{fontSize:22}}>⚙️</Text></Pressable>} /><Card onPress={()=>router.push('/schedule')}><Text style={ui.h2}>오늘의 브리핑</Text><Text style={ui.muted}>🔥 추천 행사 {all.data?.items.filter(x=>x.importance==='HIGH').length??0}개 · 🗓 참가 예정 {schedule.data?.items.length??0}개</Text><Text style={[ui.body,{fontWeight:'800',paddingTop:4}]}>내 일정/캘린더 보기 →</Text></Card><View style={ui.row}>{['전체','AI','로봇','임베디드','SW','해커톤'].map(x=><Pressable key={x} onPress={()=>setCategory(x)} style={{opacity:category===x?1:.55}}><Badge>{category===x?`✓ ${x}`:x}</Badge></Pressable>)}</View><Text style={[ui.h2,{marginTop:8}]}>🔥 놓치면 아쉬워요</Text>{all.isLoading&&<Loading/>}{all.error&&<ErrorBox message={all.error.message}/>} {items.length===0&&<Card><Text style={ui.muted}>이 필터에 맞는 행사가 아직 없습니다.</Text></Card>}{items.map(e=><EventCard key={e.id} e={e}/>)}</Page>}
