...
export type DeviceCalendarEvent={id:string;title:string;startDate:string;endDate:string;allDay:boolean;calendarId:string;calendarTitle:string;};
...
function inferredFullMonth(event:EventItem){
  const s=dateOnlyParts(event.startDate),e=dateOnlyParts(event.endDate);if(!s||!e||s.y!==e.y||s.m!==e.m||s.d!==1)return false;
  return e.d===new Date(s.y,s.m+1,0).getDate();
}
export function eventPrecision(event:EventItem):EventDatePrecision{
  if(event.datePrecision)return event.datePrecision;
  if(inferredFullMonth(event))return 'MONTH';
  return event.startDate?'EXACT':'TBD';
}
...
export function eventDateLabel(event:EventItem){
  const precision=eventPrecision(event);if(precision!=='EXACT'){
    if(event.dateText)return event.dateText;
    const p=dateOnlyParts(event.startDate);if(precision==='MONTH'&&p)return `${p.y}년 ${p.m+1}월 중`;
    return '정확한 일정 추후 발표';
  }
  const timing=getEventTiming(event);if(!timing)return '일정 확인 필요';
  if(timing.allDayLike){if(timing.multiDay)return `${formatDate(timing.start)} ~ ${formatDate(timing.displayEnd)}`;return `${formatDate(timing.start)} · 하루 종일`;}
...
export function approximateMatchesMonth(event:EventItem,month:Date){
  if(eventPrecision(event)==='EXACT')return false;const text=event.dateText??eventDateLabel(event);const m=text.match(/(\d{4})년\s*(\d{1,2})월/);return m?Number(m[1])===month.getFullYear()&&Number(m[2])===month.getMonth()+1:false;
}

export function getEventTiming(event:EventItem):EventTiming|null{
  if(!isExactEvent(event)||!event.startDate)return null;const start=parseSourceDate(event.startDate);if(Number.isNaN(start.getTime()))return null;
  ...
  if(startAllDay){let displayEnd=event.endDate?parseSourceDate(event.endDate):start;if(Number.isNaN(displayEnd.getTime())||displayEnd<start)displayEnd=start;const end=nextLocalDay(displayEnd);const durationDays=Math.max(1,Math.round((new Date(displayEnd.getFullYear(),displayEnd.getMonth(),displayEnd.getDate()).getTime()-new Date(start.getFullYear(),start.getMonth(),start.getDate()).getTime())/86400000)+1);return {start,end,displayEnd,allDayLike:true,multiDay:durationDays>1,precise:false,durationDays};}
  let end=event.endDate?parseSourceDate(event.endDate):new Date(start.getTime()+2*60*60*1000);if(Number.isNaN(end.getTime())||end<=start)end=new Date(start.getTime()+2*60*60*1000);const durationMs=end.getTime()-start.getTime();const multiDay=!sameLocalDay(start,end)||durationMs>86400000;const precise=!endAllDay&&durationMs<=86400000;return {start,end,displayEnd:end,allDayLike:false,multiDay,precise,durationDays:Math.max(1,Math.ceil(durationMs/86400000))};
}
...
export function isLikelySameEvent(event:EventItem,item:DeviceCalendarEvent){
  const a=normalizeTitle(event.title||''),b=normalizeTitle(item.title||'');if(!a||a!==b)return false;const deviceStart=new Date(item.startDate);if(Number.isNaN(deviceStart.getTime()))return false;
...
export async function loadDeviceCalendarSnapshot(start:Date,end:Date,requestIfNeeded=false):Promise<CalendarSnapshot>{
  const granted=await hasCalendarPermission(requestIfNeeded);if(!granted)return {events:[],calendarCount:0,failedCalendarCount:0};const calendars=await Calendar.getCalendarsAsync(Calendar.EntityTypes.EVENT);const usable=calendars.filter(c=>Boolean(c.id));if(!usable.length)return {events:[],calendarCount:0,failedCalendarCount:0};
  const settled=await Promise.allSettled(usable.map(async cal=>(await Calendar.getEventsAsync([cal.id],start,end)).map(item=>({item,cal}))));const out:DeviceCalendarEvent[]=[];let failed=0;
  for(const result of settled){if(result.status==='rejected'){failed++;continue;}for(const {item,cal} of result.value){const startDate=toIso(item.startDate),endDate=toIso(item.endDate??item.startDate);if(!startDate||!endDate)continue;out.push({id:`${cal.id}:${item.id}:${startDate}`,title:item.title||'제목 없는 일정',startDate,endDate,allDay:Boolean(item.allDay),calendarId:cal.id,calendarTitle:cal.title||'캘린더'});}}
  const dedup=new Map<string,DeviceCalendarEvent>();for(const item of out)dedup.set(item.id,item);return {events:[...dedup.values()].sort((a,b)=>new Date(a.startDate).getTime()-new Date(b.startDate).getTime()),calendarCount:usable.length,failedCalendarCount:failed};
}
...
export function conflictsWithEvent(event:EventItem,deviceEvents:DeviceCalendarEvent[]):EventConflict[]{
  const timing=getEventTiming(event);if(!timing)return [];const result:EventConflict[]=[];for(const item of deviceEvents){if(isLikelySameEvent(event,item))continue;const bStart=new Date(item.startDate),bEnd=new Date(item.endDate);if(Number.isNaN(bStart.getTime())||Number.isNaN(bEnd.getTime()))continue;const overlap=overlapMinutes(timing.start,timing.end,bStart,bEnd);if(overlap<=0)continue;if(timing.precise)result.push({item,minutes:overlap,approximate:false});else result.push({item,minutes:0,approximate:true});}return result.sort((a,b)=>Number(a.approximate)-Number(b.approximate)||b.minutes-a.minutes);
}
