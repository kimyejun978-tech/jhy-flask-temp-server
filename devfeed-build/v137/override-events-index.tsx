...
export default function EventsScreen() {
  ...
  const filtered = useMemo(() => all.filter(e => filter === '전체' || e.categories.some(c => c.toLowerCase().includes(filter.toLowerCase()) || filter.toLowerCase().includes(c.toLowerCase()))), [all, filter]);
  const urgent = all.filter(e => e.deadline && dday(e.deadline) && dday(e.deadline) !== '마감').sort((a, b) => new Date(a.deadline ?? 0).getTime() - new Date(b.deadline ?? 0).getTime()).slice(0, 3);
    ...
    <Page style={{ paddingTop: 10 }}>
      <View style={[styles.workspaceHero, compact && styles.workspaceHeroCompact]}>
        ...
        <View style={styles.heroStats}>
          ...
          <View style={styles.heroStatDivider} />
          <View style={styles.heroStat}><Text style={styles.heroStatNumber}>{all.length}</Text><Text style={styles.heroStatLabel}>수집됨</Text></View>
        </View>
      </View>
      ...
      <View style={styles.filterRow}>
        {FILTERS.map(item => {
          ...
          return <Pressable key={item} onPress={() => setFilter(item)} style={[styles.filterChip, active && styles.filterChipActive]}><Text style={[styles.filterText, active && styles.filterTextActive]}>{item}</Text></Pressable>;
        })}
      </View>

      <SectionTitle title={filter === '전체' ? '추천 행사' : `${filter}에서 볼 것`} subtitle="설명보다 중요한 정보부터 빠르게 볼 수 있게 정리했어요." right={!events.isLoading ? <Text style={styles.countText}>{filtered.length}</Text> : undefined} />
      {events.isLoading && <Loading />}
      {events.error && <ErrorBox message={events.error.message} />}
      {!events.isLoading && !events.error && filtered.length === 0 ? <Card><Text style={ui.muted}>이 분야에 새로 잡힌 행사가 아직 없어요.</Text></Card> : null}
      ...
      {rest.length > 0 ? <SectionTitle title="전체 목록" subtitle="최신 수집 순으로 이어서 봅니다." /> : null}
