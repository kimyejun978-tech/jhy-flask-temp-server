...
export default function Trends(){
 ...
 const all=q.data?.items??[];const items=useMemo(()=>selected==='전체'?all:all.filter(i=>categoriesOf(i).includes(selected)),[all,selected]);
 ...
 return <Page>
  ...
  <SectionTitle title={selected==='전체'?'오늘 읽을 것':selected} subtitle="카드보다 빠르게 훑을 수 있는 인박스형 목록" right={!q.isLoading?<Text style={styles.count}>{items.length}</Text>:undefined}/>
  ...
  <View style={styles.list}>{items.map((x,index)=>{const cats=categoriesOf(x);return <Pressable key={x.id} onPress={()=>router.push(`/trend/${x.id}`)} style={({pressed})=>[styles.row,pressed&&{opacity:.6}]}>
    <View style={styles.numberCol}><Text style={styles.number}>{String(index+1).padStart(2,'0')}</Text><View style={styles.line}/></View>
    <View style={styles.rowBody}>
     <View style={styles.rowHead}><View style={styles.badges}>{cats.slice(0,compact?1:2).map(c=><Badge tone="purple" key={c}>{c}</Badge>)}</View><Text style={styles.date}>{dateText(x.publishedAt)}</Text></View>
