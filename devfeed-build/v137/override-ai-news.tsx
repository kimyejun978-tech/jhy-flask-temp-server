...
export default function AiNews(){
 const {width}=useWindowDimensions();const compact=width<370;const q=useNews();const items=q.data?.items??[];const ready=items.filter(x=>Boolean(x.summary));const pending=items.length-ready.length;const lead=ready[0]??items[0];const rest=lead?items.filter(x=>x.id!==lead.id):items;
 return <Page>
  ...
  {lead?<><SectionTitle title="오늘의 헤드라인" subtitle="먼저 읽을 만한 한 편"/><Pressable onPress={()=>router.push(`/ai-news/${lead.id}`)} style={({pressed})=>[styles.leadCard,pressed&&{opacity:.65}]}>
   <View style={styles.leadMeta}><Badge tone="purple">{lead.channel}</Badge><Text style={styles.date}>{dateText(lead.publishedAt)}</Text></View>
  ...
  {rest.length>0?<><SectionTitle title="뉴스 인박스" subtitle="최신 영상과 요약을 한 줄씩 확인하세요."/><View style={styles.list}>{rest.map((x,index)=><Pressable key={x.id} onPress={()=>router.push(`/ai-news/${x.id}`)} style={({pressed})=>[styles.row,pressed&&{opacity:.6}]}>
   <View style={styles.indexBox}><Text style={styles.indexText}>{String(index+1).padStart(2,'0')}</Text></View>
   <View style={styles.rowBody}><View style={styles.rowMeta}><Text style={styles.channel}>{x.channel}</Text><Text style={styles.date}>{dateText(x.publishedAt)}</Text></View><Text style={styles.title}>{x.title}</Text><Text numberOfLines={2} style={styles.summary}>{x.summary??'요약 생성 중 · 완료되면 자동으로 갱신됩니다.'}</Text><View style={styles.statusRow}><View style={[styles.statusDot,{backgroundColor:x.summary?colors.green:colors.orange}]}/><Text style={styles.statusText}>{x.summary?'요약 완료':'처리 중'}</Text></View></View>
