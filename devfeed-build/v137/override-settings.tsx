...
export default function Settings(){
 const {width}=useWindowDimensions();const compact=width<370;const q=usePreferences();const save=useSavePreferences();const [p,setP]=useState<Preferences>(defaults);const [permission,setPermission]=useState('확인 중');const [profileId,setProfileId]=useState('불러오는 중');const [updateStatus,setUpdateStatus]=useState('확인 안 함');const [updateInfo,setUpdateInfo]=useState<UpdateInfo|null>(null);const [installing,setInstalling]=useState(false);
 ...
 return <Page>
   ...
   <Card><View style={styles.updateHead}><View><Text style={styles.updateTitle}>DevFeed</Text><Text style={styles.updateVersion}>현재 v{currentAppVersion()}</Text></View><Badge tone={updateInfo?'orange':'neutral'}>{updateStatus}</Badge></View><Button label="업데이트 확인" variant="secondary" onPress={()=>void checkUpdate()}/>{updateInfo?<Button label={installing?'다운로드 중…':`v${updateInfo.latestVersion} 설치`} disabled={installing} onPress={()=>void installUpdate()}/>:null}</Card>
