  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[
        styles.pageContent,
        { paddingHorizontal: horizontal, paddingTop: Math.max(insets.top + 8, 16), paddingBottom: Math.max(insets.bottom + 110, 118) },
        style,
      ]}
...
export function Button({ label, onPress, variant = 'primary', disabled = false }: { label: string; onPress: () => void; variant?: 'primary' | 'secondary' | 'ghost' | 'danger'; disabled?: boolean }) {
  ...
  return <Pressable disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.button, buttonStyle, (pressed || disabled) && { opacity: 0.58 }]}><Text style={textStyle}>{label}</Text></Pressable>;
}
