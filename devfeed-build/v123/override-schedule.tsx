...
import { conflictsWithEvent, describeEventTiming, getEventTiming, hasCalendarPermission, isLikelySameEvent, loadDeviceCalendarSnapshot, type DeviceCalendarEvent } from '@/calendar';
...
function keysForDevFeed(event:EventItem){
  const timing=getEventTiming(event);
  if(!timing)return [];
  if(timing.durationDays>7){
    const first=dayKey(timing.start);const last=dayKey(timing.displayEnd);
    return [...new Set([first,last].filter(Boolean))];
  }
  return keysForRange(timing.start.toISOString(),timing.end.toISOString(),timing.allDayLike);
}
...
export default function ScheduleScreen(){
  ...
  const visibleDeviceEvents=useMemo(()=>deviceEvents.filter(item=>!devfeed.some(e=>isLikelySameEvent(e,item))),[deviceEvents,schedule.data]);

  const {devfeedByDay,deviceByDay}=useMemo(()=>{
    const d=new Map<string,EventItem[]>();
    const g=new Map<string,DeviceCalendarEvent[]>();
    for(const e of devfeed)addToMap(d,keysForDevFeed(e),e);
    for(const e of visibleDeviceEvents)addToMap(g,keysForRange(e.startDate,e.endDate,e.allDay),e);
    return {devfeedByDay:d,deviceByDay:g};
  },[schedule.data,visibleDeviceEvents]);

  const dayDev=devfeedByDay.get(selected)??[];const dayDevice=deviceByDay.get(selected)??[];
  const conflictMap=new Map<string,{minutes:number;approximate:number;count:number}>();
  for(const e of dayDev){
    const conflicts=conflictsWithEvent(e,deviceEvents);
    if(!conflicts.length)continue;
    conflictMap.set(e.id,{minutes:conflicts.filter(x=>!x.approximate).reduce((sum,x)=>sum+x.minutes,0),approximate:conflicts.filter(x=>x.approximate).length,count:conflicts.length});
  }
  ...
  return <Page>
    ...
    <Card>
      ...
      <Text style={ui.muted}>Google·Samsung Calendar와 DevFeed 참가 일정을 한 화면에서 봅니다. 캘린더 내용은 서버로 보내지 않습니다.</Text>
      {permission==='checking'&&<Text style={ui.muted}>캘린더 권한 확인 중…</Text>}
      {permission==='denied'&&<View style={{gap:8}}><Text style={ui.muted}>기기 일정을 같이 보려면 캘린더 접근 권한이 필요합니다. 권한이 없어도 DevFeed 일정은 볼 수 있어요.</Text><Button label="캘린더 접근 허용" onPress={()=>void load(true)}/><Button label="휴대폰 앱 설정 열기" variant="ghost" onPress={()=>void Linking.openSettings()}/></View>}
      ...
      {loadError&&<View style={{gap:8}}><Text style={{color:colors.red,fontWeight:'800'}}>캘린더를 불러오지 못했습니다.</Text><Text style={ui.muted}>DevFeed 일정은 계속 사용할 수 있습니다. 다시 시도해도 안 되면 휴대폰의 캘린더 동기화/권한을 확인해주세요.</Text><Button label="다시 불러오기" variant="ghost" onPress={()=>void load(false)}/></View>}
    ...
    {dayDev.map(e=>{const conflict=conflictMap.get(e.id);const alsoOnDevice=deviceEvents.some(x=>isLikelySameEvent(e,x));return <Card key={`d-${e.id}`} onPress={()=>router.push(`/event/${e.id}`)}><View style={{flexDirection:'row',justifyContent:'space-between',gap:8}}><Text style={[ui.h2,{flex:1}]}>{e.title}</Text><Badge>DevFeed</Badge></View><Text style={ui.muted}>{describeEventTiming(e)} · {e.isOnline?'온라인':e.location??'장소 미정'}</Text>{alsoOnDevice&&<Text style={{color:colors.green,fontWeight:'800'}}>✓ 기기 캘린더에도 추가됨</Text>}{conflict?.approximate?<Text style={{color:colors.red,fontWeight:'900'}}>⚠ 같은 기간에 기존 일정 {conflict.count}개 있음 · 정확한 시간이 없어 시간 단위 충돌은 계산하지 않음</Text>:conflict&&conflict.minutes>0?<Text style={{color:colors.red,fontWeight:'900'}}>⚠ 기존 일정과 약 {conflict.minutes>=60?`${Math.floor(conflict.minutes/60)}시간${conflict.minutes%60?` ${conflict.minutes%60}분`:''}`:`${conflict.minutes}분`} 겹쳐요</Text>:null}</Card>})}
