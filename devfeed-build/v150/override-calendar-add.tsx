...
export default function CalendarAddScreen() {
  ...
  const naturalPreview = useMemo(() => {
    ...
    try { return { draft: parseNaturalCalendarText(naturalText), error: '' }; }
    catch (e) { return { draft: null, error: e instanceof Error ? e.message : '입력을 해석하지 못했습니다.' }; }
  }, [naturalText]);
    ...
    return (
      <Page style={{ paddingTop: 8, paddingBottom: 40 }}>
        <View style={styles.quickHeader}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}><Text style={styles.quickEyebrow}>DEVFEED QUICK ADD</Text><Text style={styles.quickTitle}>일정을 바로 적어보세요.</Text></View>
        </View>
        ...
        <View style={styles.searchInputShell}>
          ...
          <TextInput
            ...
            placeholder="예) 내일 3시 수학 수행"
            placeholderTextColor="#8D8A96"
            style={styles.quickInput}
            returnKeyType="done"
            blurOnSubmit={false}
            onSubmitEditing={() => { if (!busy && naturalText.trim()) void saveNatural(); }}
          />
          <Pressable disabled={busy || !naturalPreview.draft} onPress={() => { void saveNatural(); }} style={[styles.quickGo, (!naturalPreview.draft || busy) && { opacity: 0.35 }]}>
            <Text style={styles.quickGoText}>→</Text>
          </Pressable>
        </View>

        <Text style={styles.quickHint}>문장을 적고 키보드의 완료를 누르면 바로 캘린더에 저장돼요.</Text>

        {naturalText.trim() ? <Card style={styles.quickPreview}><Text style={styles.cardEyebrow}>PREVIEW</Text>{naturalPreview.draft ? <><Text style={styles.previewTitle}>{naturalPreview.draft.title}</Text><Text style={styles.previewDate}>{previewDate(naturalPreview.draft)}</Text></> : <Text style={styles.errorText}>{naturalPreview.error}</Text>}</Card> : null}
  ...
  return (
    <Page style={{ paddingBottom: 70 }}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}><Text style={styles.eyebrow}>QUICK ADD</Text><Text style={styles.title}>일정 추가</Text><Text style={styles.subtitle}>말하듯 적거나, 날짜와 시간을 직접 입력해서 휴대폰 캘린더에 바로 저장해요.</Text></View>
      </View>

      {fromWidget ? <Badge tone="purple">홈 화면 위젯에서 열었어요</Badge> : null}
      <View style={styles.switcher}>
        <Pressable onPress={() => setMode('natural')} style={[styles.switchItem, mode === 'natural' && styles.switchItemActive]}><Text style={[styles.switchText, mode === 'natural' && styles.switchTextActive]}>자연어로 추가</Text></Pressable>
        <Pressable onPress={() => setMode('manual')} style={[styles.switchItem, mode === 'manual' && styles.switchItemActive]}><Text style={[styles.switchText, mode === 'manual' && styles.switchTextActive]}>직접 입력</Text></Pressable>
      </View>

      {mode === 'natural' ? <>
        <Card style={styles.naturalCard}>
          <Text style={styles.cardEyebrow}>NATURAL LANGUAGE</Text><Text style={styles.cardTitle}>그냥 말하듯 적어보세요.</Text>
          <TextInput autoFocus value={naturalText} onChangeText={setNaturalText} multiline placeholder="예) 내일 오후 3시 수학 수행평가" placeholderTextColor="#9B98A7" style={styles.naturalInput} returnKeyType="done" />
          <View style={styles.exampleWrap}>{EXAMPLES.map(example => <Pressable key={example} onPress={() => setNaturalText(example)} style={styles.exampleChip}><Text style={styles.exampleText}>{example}</Text></Pressable>)}</View>
        </Card>
        {naturalText.trim() ? <Card><Text style={styles.cardEyebrow}>PREVIEW</Text>{naturalPreview.draft ? <><Text style={styles.previewTitle}>{naturalPreview.draft.title}</Text><Text style={styles.previewDate}>{previewDate(naturalPreview.draft)}</Text><Text style={styles.previewHint}>필요하면 위 문장을 고치면 미리보기도 같이 바뀝니다.</Text></> : <Text style={styles.errorText}>{naturalPreview.error}</Text>}</Card> : null}
        <Button label={busy ? '저장 중…' : '이 일정 저장'} onPress={() => { void saveNatural(); }} disabled={busy} />
      </> : <>
        <Card><Text style={styles.cardEyebrow}>MANUAL</Text><Text style={styles.cardTitle}>날짜와 시간을 직접 정해요.</Text>
      ...
      <Text style={styles.footer}>DevFeed는 새 일정을 기기의 수정 가능한 캘린더에 저장합니다.</Text>
    </Page>
  );
}

const styles = StyleSheet.create({
  quickHeader: { flexDirection: 'row', gap: 12, alignItems: 'center', marginBottom: 6 },
  quickClose: { width: 42, height: 42, borderRadius: 15, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, alignItems: 'center', justifyContent: 'center' },
  quickCloseText: { color: colors.text, fontSize: 31, lineHeight: 34, marginTop: -2 },
  quickEyebrow: { color: colors.accent, fontSize: 9, fontWeight: '900', letterSpacing: 1.6 },
  quickTitle: { color: colors.text, fontSize: 22, lineHeight: 28, fontWeight: '900', letterSpacing: -0.5, marginTop: 2 },
  searchInputShell: { minHeight: 66, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, backgroundColor: '#FFFFFF', borderRadius: 28, borderWidth: 1, borderColor: '#DDD9E6' },
  searchMark: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center' },
  searchMarkText: { color: '#fff', fontSize: 25, lineHeight: 28, fontWeight: '500' },
  quickInput: { flex: 1, minWidth: 0, height: 56, color: colors.text, fontSize: 16, fontWeight: '700' },
  quickGo: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#EEE8FF', alignItems: 'center', justifyContent: 'center' },
  quickGoText: { color: colors.accentDark, fontSize: 21, fontWeight: '900' },
  quickHint: { color: colors.muted, fontSize: 11, lineHeight: 17, paddingHorizontal: 8 },
  quickPreview: { paddingVertical: 14 },
  quickExamples: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 },
  manualLink: { minHeight: 44, alignItems: 'center', justifyContent: 'center' },
  ...
  exampleChip: { backgroundColor: 'rgba(255,255,255,0.82)', borderRadius: radius.pill, paddingHorizontal: 10, paddingVertical: 7 },
  ...
  fieldWrap: { gap: 6 }, fieldLabel: { color: colors.text, fontSize: 12, fontWeight: '900' }, fieldHint: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 2 },
  ...
  multiline: { minHeight: 92, paddingTop: 13, textAlignVertical: 'top' }, timeRow: { flexDirection: 'row', gap: 10 }, allDayRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 4 },
